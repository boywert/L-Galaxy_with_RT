...
void add_infall_to_hot(int centralgal, double infallingGas) {
	double remove_ejected=0., remove_hot=0.;
  ...
  /*if (infallingGas < 0.) {
	...
	//if infall is negative take gas from ejected, then from hot, checking
	//not more than its available its removes
	if(infallingGas<0.)
	{
		if(fabs(infallingGas)>Gal[centralgal].EjectedMass)
		{
			remove_ejected=Gal[centralgal].EjectedMass;
			remove_hot=fabs(infallingGas)-remove_ejected;
			if(remove_hot>Gal[centralgal].HotGas)
				remove_hot=Gal[centralgal].HotGas;
		}
		else
			remove_ejected=fabs(infallingGas);

		Gal[centralgal].EjectedMass -= remove_ejected;
		Gal[centralgal].HotGas -= remove_hot;
	}
	else	//  Add the infalling gas to the central galaxy hot component
