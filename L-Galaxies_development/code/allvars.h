...
#endif
  float BulgeMass; // 10^10/h Msun - Mass in the bulge
  float DiskMass;  //disk stellar mass, no bulge
#ifdef H2_AND_RINGS
  float DiskMassr[RNUM];
#endif
  float HotGas; // 10^10/h Msun - Mass in hot gas
...
struct GALAXY			/* Galaxy data */
{
  ...
  double ColdGas;
  double ColdGasr[RNUM];
  float BulgeMass;
  double DiskMass;
  double DiskMassr[RNUM];
  float ReheatedGas;
  float ReheatedL; // The angular momentum in reheated gas
  float HotGas;
  float EjectedMass;
  float BlackHoleMass;
  float BlackHoleGas;
  double H2fraction;
  double H2fractionr[RNUM];
  ...
  double MetalsColdGasr[RNUM];
  double MetalsBulgeMass;
  double MetalsDiskMass;
  double MetalsDiskMassr[RNUM];
  float MetalsHotGas;
  ...
  float MergRadius;
  float MergRadiusconst;
  float Mergmass;
  float CentralVelDisp;
#endif
  float ICM;
 #ifdef DETAILED_METALS_AND_MASS_RETURN
